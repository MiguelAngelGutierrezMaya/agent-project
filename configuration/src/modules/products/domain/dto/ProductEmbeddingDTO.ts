...
import type { ProductEmbedding } from '@modules/products/domain/models/ProductEmbedding';

/**
 * Product Embedding DTO
 *
 * @description
 * Data Transfer Object for product embedding data.
...
export class ProductEmbeddingDTO {
  ...
  static create(
    ...
    const id = data.id;
    const productId = (data.productId || data.product_id) as string;
    ...
    if (
      ...
      return [
        undefined,
        new DomainValidationError('Invalid product embedding data'),
      ];
    }
